...
def get_klines_five_minute(tick):
    try:
...
def get_klines_one_hour(tick):
    try:
        klines = client.futures_klines(symbol=tick, interval=Client.KLINE_INTERVAL_1HOUR, limit=48)
        timeframe = "1 Hours"
    except Exception as e:
        print(f"Error while get data for klines  {tick}: {e}")
        return None
...
def get_klines_four_hour(tick):
    try:
        klines = client.futures_klines(symbol=tick, interval=Client.KLINE_INTERVAL_4HOUR, limit=48)
        timeframe = "4 Hours"
    except Exception as e:
        print(f"Error while get data for klines  {tick}: {e}")
        return None
...
def get_bollinger_signals(tick, klines, timeframe):
    ...
    if min_low < lower_band.iloc[-1]:
        ...
        if  volumen >= ideal_volumen:
            send_telegram_message("⚠️ Third Bollinger Bands Broken BTC", timeframe, "Possible Long", tick, human_format(volumen), info['lastPrice'], info['highPrice'], info['lowPrice'], False)
            return True
    ...
    # SHORT signals
        ...
        volumen = float(info['quoteVolume'])
        if  volumen >= ideal_volumen:
            send_telegram_message("⚠️ Third Bollinger Bands Broken BTC", timeframe, "Possible Short", tick, human_format(volumen), info['lastPrice'], info['highPrice'], info['lowPrice'], False)
            return True
...
def main_loop():
    while True:
        ...
        print('')
        for tick in ticks:
            klines_5m, time5m = get_klines_five_minute(tick)
            if klines_5m is not None:
                found_signal_bollinger_5m = get_bollinger_signals(tick, klines_5m, time5m)
                if found_signal_bollinger_5m:
                    print("Found signal for", tick, "on 5 minute timeframe")
                    print('**************************************************')
                    print('')
